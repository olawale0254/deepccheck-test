...
from .models import Interaction, Metric, Alert
...
def get_metrics(db: Session = Depends(get_db)):
    ...
    return [{"interaction_id": m.interaction_id, "metric_name": m.metric_name, "input_value": m.input_value, "output_value": m.output_value} for m in metrics]

@router.get("/alerts", response_model=list)
def get_alerts(db: Session = Depends(get_db)):
    alerts = db.query(Alert).all()
    return [{"interaction_id": a.interaction_id, "metric_name": a.metric_name, "element": a.element, "alert_type": a.alert_type, "value": a.value} for a in alerts]
